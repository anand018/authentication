...
public class AccountController {
    ...
    public String saveProfilePicture(@PathVariable String username, @RequestPart("file") MultipartFile file) throws IOException {
        return accountService.saveProfilePicture(username, file.getBytes());
    }
    @GetMapping("/profile-picture/{username}")
    public ResponseEntity<byte[]> getProfilePicture(@PathVariable String username) {
        AccountEntity accountEntity = accountService.getProfilePicture(username);
        return ResponseEntity.ok().body(accountEntity.getProfilePicture());
    }
}